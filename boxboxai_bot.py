...
def get_utc_offset_hours(tz_name: str) -> float:
    """Returns the current UTC offset (hours) for an IANA timezone name, DST-aware."""
    import datetime as _dt
    offset = _dt.datetime.now(_dt.timezone.utc).astimezone(ZoneInfo(tz_name)).utcoffset()
    return offset.total_seconds() / 3600

# Comprehensive country → IANA timezone map
COUNTRY_TIMEZONES = {
    # Americas
    "mexico": ("America/Mexico_City", "🇲🇽 Mexico"),
    "méxico": ("America/Mexico_City", "🇲🇽 Mexico"),
    "mexico city": ("America/Mexico_City", "🇲🇽 Mexico"),
    "guadalajara": ("America/Mexico_City", "🇲🇽 Mexico"),
    "monterrey": ("America/Monterrey", "🇲🇽 Mexico"),
    "usa": ("America/New_York", "🇺🇸 USA (East)"),
    "united states": ("America/New_York", "🇺🇸 USA (East)"),
    "new york": ("America/New_York", "🇺🇸 USA (East)"),
    "miami": ("America/New_York", "🇺🇸 USA (East)"),
    "chicago": ("America/Chicago", "🇺🇸 USA (Central)"),
    "dallas": ("America/Chicago", "🇺🇸 USA (Central)"),
    "houston": ("America/Chicago", "🇺🇸 USA (Central)"),
    "denver": ("America/Denver", "🇺🇸 USA (Mountain)"),
    "los angeles": ("America/Los_Angeles", "🇺🇸 USA (Pacific)"),
    "california": ("America/Los_Angeles", "🇺🇸 USA (Pacific)"),
    "seattle": ("America/Los_Angeles", "🇺🇸 USA (Pacific)"),
    "canada": ("America/Toronto", "🇨🇦 Canada (East)"),
    "toronto": ("America/Toronto", "🇨🇦 Canada (East)"),
    "montreal": ("America/Toronto", "🇨🇦 Canada (East)"),
    "vancouver": ("America/Vancouver", "🇨🇦 Canada (West)"),
    "brazil": ("America/Sao_Paulo", "🇧🇷 Brazil"),
    "brasil": ("America/Sao_Paulo", "🇧🇷 Brazil"),
    ...
    "deutschland": ("Europe/Berlin", "🇩🇪 Germany"),
    "italy": ("Europe/Rome", "🇮🇹 Italy"),
    "italia": ("Europe/Rome", "🇮🇹 Italy"),
    ...
    # Oceania
    "australia": ("Australia/Sydney", "🇦🇺 Australia (East)"),
    "sydney": ("Australia/Sydney", "🇦🇺 Australia (East)"),
    "melbourne": ("Australia/Melbourne", "🇦🇺 Australia (East)"),
    "perth": ("Australia/Perth", "🇦🇺 Australia (West)"),
...
def lookup_country_tz(text: str) -> tuple | None:
    ...
    if t in COUNTRY_TIMEZONES:
        tz_name, label = COUNTRY_TIMEZONES[t]
        return (get_utc_offset_hours(tz_name), label)
    # Partial match
    for key, (tz_name, label) in COUNTRY_TIMEZONES.items():
